export const BG = {
  firstName: "Име",
  lastName: "Фамилия",
  email: "Имейл",
  phone: "Телефон",
  home: "Новини",
  lessons: "Уроци",
  login: "Вход",
  logout: "Изход",
  register: "Регистрация",
  forgotten: "Забравена парола",
  password: "Парола",
  news: "Новини",
  for: "За",
  age: "Възраст",
  skills: "Умения",
  participants: "Участници",
  type: "Вид",
  count: "Брой участници",
  location: "Местоположение",
  price: "Цена",
  more: "Още за този урок",
  profile: "Профил",
  back: "Назад",
  edit: "Редактирай",
  skaters: "Кънкьори",
  history: "История",
  hello: "Здравей",
  update: "Обнови профила",
  dashboard: "Табло",
  details: "Още",
  send: "Изпрати",
  addSkater: "Добави кънкьора",
  addSkaters: "Добави кънкьорите",
  level: "Ниво",
  levelEn: "Ниво на Английски език",
  levelBg: "Ниво на Български език",
  description: "Описание",
  descriptionEn: "Описание на Английски език",
  descriptionBg: "Описание на Български език",
  typePaymentEn: "Тип абонамент на Английски език",
  typePaymentBg: "Тип абонамент на Български език",
  editSkater: "Редактирай",
  deleteSkater: "Изтрий",
  deleteSkaterQuestion: "Желаете ли да изтриете",
  requirements: "Допълнителни изисквания",
  noAddedSkaters: "Все още няма добавени кънкьори!",
  settings: "Настройки",
  adminPanel: "Админ панел",
  subscription: "Вид урок",
  subscriptionCount: "Брой посещения за абонамент",
  visit: "посещени", // Do not add any letter is used for singular and plural (client/src/features/booking/UnregisteredUser.jsx --> subscriptionData)
  ageGroup: "Възраст",
  // ageGroup: "Възрастова група",
  noAddedOptions: "Няма добавени опции!",
  addOptions: "Добави опция",
  editOptions: "Редактирай опция",
  deleteOptions: "Изтрий опция",
  quantity: "Количество",
  number: "Номер",
  booking: "Запиши се",
  bookLesson: "Записване за урок",
  skates: "Кънки",
  protection: "Протектори",
  haveOwn: "Имам свои",
  hasOwn: "Има свои",
  contactName: "Име за контакт",
  select_skaters: "Изберете кънкьори",
  schedule: "График",
  title: "Име",
  onBul: " на Български",
  visits: "Брой посещения",
  unregistered_user_form_title: "Попълнете формата",
  latitude: "Географска ширина",
  longitude: "Географска дължина",
  time: "Час",
  imageUrl: "Адрес на изображение",
  validTo: "Дата на валидност",
  learnMore: "Научете повече",
  activeDays: "Редовни уроци",
  individualDays: "Индивидуални уроци",
  mon: "Понеделник",
  tue: "Вторник",
  wed: "Сряда",
  thu: "Четвъртък",
  fri: "Петък",
  sat: "СЪбота",
  sun: "Неделя",
  team: "Екипът",
  agree: "Съгласявам се с",
  terms: "Общите условия",
  haveAccount: "Вече сте се регистрирали?",
  doNotHaveAccount: "Нямате регистрация?",
  signIn: "Влез!",
  signUp: "Регистрирай се!",
  onDuty: "Настоящи уроци",
  from: "От",
  to: "До",
  date: "Дата",
  sequence: "Поредност",
  reject: "Откажи",
  book: "Запиши се",
  about: "За Нас",
  years: "години",
  terms_1: "Съгласен съм с ",
  terms_2: "Общите условия",
  restriction: "Само регистрирани потребители могат да записват абонамент!",
  cancelDate: "Анулирай",
  repass: "Повтори паролата",

  // ADMIN OPTIONS
  a_skates_info_1: "Добавете номер и количество",
  a_skates_info_2:
    "Имате възможност да прегледате и редактирате всеки един от записите в списъка.",
  a_skates_info_3: "Сортирането става автоматично, във възходящ ред",
  a_skates_info_4:
    "За опция „Имам собствени“ добавете 0 (нула). Ще се визуализира като текст",

  a_info_1:
    "В тази секция имате възможност за преглед, редакция и добавяне целия наличен инвентар",
  a_info_2: "Промяна и добавяне на абонаментен план и възрастова категория",
  a_info_3: "За достъп изберете от менюто",
  a_protection:
    "Полето за размер при записване ще бъде конвертирано в главни букви",
  a_level_1:
    "За максимална функционалност на приложението добавете и двете опции за език",
  a_level_2:
    "Описанието не е задължително, но ще е полезно при преглед на категорията",
  a_list_level_1: "Добавете тип (начинаещи, напреднали) група и описание.",
  a_list_level_2:
    "Имате възможност да прегледате и редактирате всеки един от записите в списъка.",
  a_active_days: "Дни за уроци",
  a_lessons: "Уроци",
  a_selectedDays_regular:
    "Тук може да изберете дните, в които ще се провеждат уроците. Тази селекция ще е валидна само за редовните уроци. Няма да важи за индивидуалните",
  a_selectedDays_individual:
    "Тук може да изберете дните, в които ще се провеждат уроците. Тази селекция ще е валидна само за индивидуалните уроци.",
  a_selectedDays_election: "Избраните дни за тренировки са:",
  a_team_management: "Екип",
  a_user_management: "Потребители",
  a_all: "Всички",
  a_instructors: "Инструктори",
  a_admins: "Администратори",
  a_search: "Търси",
  a_users_p1: "Търси във всички регистрирани потребители",
  a_available_roles: "Възможни роли",
  a_user: "Потребител",
  a_instructor: "Инструктор",
  a_admin: "Администратор",
  a_change: "Смени",
  a_select: "Избери",
  a_dates_0: "Избери колко дни преди урок формата да не е активна",
  a_dates_1: "Текущите избрани дни са :",
  a_dates_2: "Избери определена дата за да я изключиш от графика",
  a_dates_3: "Може да изберете повече от една дата едновременно",
  a_dates_4: "След изтичане на избрана дата, автоматично ще отпадне от списъка",
  a_dates_5: "Полето приема потребителски данни",
  a_dates_6: "Изключените дати са: ",
  a_protection_info_1: "Добавете номер и количество",
  a_protection_info_2:
    "Можете да преглеждате, редактирате и изтриете артикула на всички",
  a_protection_info_3:
    "Списъка ще се вижда от потребителите в реда на въвеждане. Ще е добра идея да ги въведете в желания за показване ред",
  a_age_options_info_1: "Въведените опции ще се извеждат на потребителите",
  a_age_options_info_2: "Препоръчителен формат за въвеждане - 5 - 10",
  a_age_options_info_3:
    "Можете да преглеждате, редактирате и изтриете всеки елемент",
  a_subscription_info_1:
    "Добавете тип на урока (Единичен урок, Абонамент ...), броя посещения и цена.",
  a_subscription_info_2:
    "Цената ще се вижда само от администраторите. Това е служебно поле",
  a_subscription_info_3:
    "Имате възможност да прегледате и редактирате всеки един от записите в списъка.",

  // SKATER OPTIONS
  s_firstName: "Име",
  s_lastName: "Фамилия",
  s_skates: "Кънки",
  s_protections: "Протектори",
  s_genderMale: "Мъж",
  s_genderFemale: "Жена",
  s_requirements: "Допълнително",
  s_list_1:
    "Кънкьорите, които добавите и виждате тук ще са достъпни от страницата за записване на уроци. Необходимото оборудване ще е автоматично включено към избраната тренировка.",
  s_list_2:
    "Имате възможност да преглеждате и редактирате всеки един от кънкьорите в списъка.",
  s_lessons: "Уроци",

  // USER OPTIONS
  u_add_skater_info:
    "Ако имате допълнителни изисквания или персонализирани нужди, добавете ги към полето 'Допълнителни изисквания'.",
  u_no_lessons: "Нямате записани уроци",
  u_info_1:
    "Уроците, за които сте се записали са подредени по последователност на датите",
  u_info_2:
    "В случай на невъзможност да присъствате на урока, моля, използвайте бутона 'Откажи'.",
  u_info_3:
    "Ако урокът е от абонамент, следвайки изискванията на клуба, имате право на един пропуск. Уроците се изместват с една седмица напред. С всеки следващ отказ губите съответния урок.",

  // INSTRUCTOR

  i_signedLessons: "Записани уроци",
  i_equipment: "Оборудване",
  i_noNeededEquipment: "За този урок не е необходимо оборудване",
  i_noSignedLessons: "Няма записани уроци",
  i_cancelLable: "Съобщение до потребителите",
  i_cancelLesson: "Анулирай урока",

  // Book lessons
  book_headInfo_1:
    "Преди да попълните формата, моля, запознайте се с видовете ",
  book_headInfo_2: "и техния ",
  book_sec_heading_1: "Следвайте стъпките",
  book_sec_heading_2: "Изберете ден",

  l_info_1:
    "Индивидуалните уроци се провеждат и уговарят директно с инструктора.",
  l_activeLesson_info_1:
    "означава, че кънкьора разполага със собствено оборудване",
};
