...
function AddSkatesOptions({ onClose, refreshState }) {
    const { lang } = useLanguage();

    const { mutate, isPending } = useAddOptionsQuery(refreshState);

    const { register, handleSubmit, reset } = useForm();

    // SUBMITTING THE FORM
    function onFormSubmit(skatesData) {
        mutate({ ...skatesData, optionNameData: 'skatesData' });
        onClose();
        reset();
    }
    ...
    return (
        <Popup onClose={onClose} backgroundClick={false}>
            ...
            <div className={styles.container}>
                ...
                <h2 className={styles.heading}>{lang.addSkatesOptions}</h2>
                <form
                ...
                >
                    ...
                    <div className={styles.btnContainer}>
                        <div style={{ marginLeft: "auto" }}>
                            <Button type={"primary"}>{lang.addSkatesOptions}</Button>
                        </div>
                    </div>
                </form>
